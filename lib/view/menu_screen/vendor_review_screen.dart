  ...
  void firebaseInit(BuildContext context) {
    FirebaseMessaging.onMessage.listen((message) async {
      if (message.data["actions"] == "Review") {
        ...
        if (widget.vId == data["vendor_id"]) {
          if (mounted) {
            setState(() {
              GlobalVariables.nottifyCount > 0
                  ? GlobalVariables.nottifyCount =
                      GlobalVariables.nottifyCount - 1
                  : GlobalVariables.nottifyCount = 0;
              seenNotifications(message.data["noti_id"]);
              getVenderReviews();
            });
          }
        } else {}
...
//////////////////////////////////////
  ...
  sendReview() async {
    ...
    try {
      ...
      if (response.statusCode == StatusCode.OK) {
        showReviewDialog(context, "Thanks for the Review!");
        getVenderReviews();
        setState(() {
  ...
  Future<void> commentsReply(index) async {
    ...
    try {
      ...
      if (response.statusCode == StatusCode.OK) {
        var data = response.data;
        showReviewDialog(context, 'Replied Successfully!');
        getVenderReviews();
      } else {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Something went wrong, please try again!'),
